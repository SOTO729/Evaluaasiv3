/**
 * Detalle de Ciclo Escolar - Mi Plantel (Responsable)
...
import {
  getMiPlantelCycleDetail,
  SchoolCycle,
} from '../../services/partnersService';
import { useAuthStore } from '../../store/authStore';

export default function MiPlantelCicloDetailPage() {
  const { cycleId } = useParams();
  const navigate = useNavigate();
  const { user } = useAuthStore();
  const canManage = user?.can_manage_groups;
  ...
  const loadCycle = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const cycleData = await getMiPlantelCycleDetail(parseInt(cycleId));
      setCycle(cycleData);
    } catch (err: any) {
            ...
            {searchQuery ? (
              ...
              <>
                ...
                <p className="fluid-text-base text-gray-500 fluid-mb-6">
                  {canManage ? 'Crea el primer grupo para comenzar' : 'Aún no se han creado grupos en este ciclo'}
                </p>
                {canManage && (
                  <button
                    onClick={() => navigate(`/mi-plantel/grupos/nuevo?cycleId=${cycle.id}`)}
