...
interface RequestLine {
  ...
  groupId: number | null; // null only before selection
  groupName: string;
  units: number;
  pricePerUnit: number;
...
export default function SolicitarBecaPage() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await createBatchBalanceRequest({
                            ...
                            <tr key={line.id} className="hover:bg-gray-50/50 transition-colors">
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                  <div className={`p-1.5 rounded-lg ${line.type === 'saldo' ? 'bg-emerald-100' : 'bg-primary-100'}`}>
                                    {line.type === 'saldo' ? (
                                  ...
                                  <span className={`font-medium ${line.type === 'saldo' ? 'text-emerald-700' : 'text-primary-700'}`}>
                ...
                <>
                  ...
                  {searchTerm.trim() && (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-purple-50 text-purple-700 rounded-lg text-xs font-medium">
                      "{searchTerm}"
                      <button onClick={() => setSearchTerm('')} aria-label="Quitar búsqueda" className="hover:text-purple-900"><X className="w-3 h-3" /></button>
                    </span>
                  )}
                  {partnerFilter && (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-purple-50 text-purple-700 rounded-lg text-xs font-medium">
                      {partnerFilter}
                      <button onClick={() => setPartnerFilter('')} aria-label="Quitar partner" className="hover:text-purple-900"><X className="w-3 h-3" /></button>
                    </span>
                  )}
                  {stateFilter && (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-purple-50 text-purple-700 rounded-lg text-xs font-medium">
                      {stateFilter}
                      <button onClick={() => setStateFilter('')} aria-label="Quitar estado" className="hover:text-purple-900"><X className="w-3 h-3" /></button>
                    </span>
                  )}
                  <button
                    type="button"
            ...
            {requestLines.length === 0 ? (
              ...
              <>
                    ...
                    <div key={line.id} className="flex items-center justify-between text-sm py-1.5 px-2 rounded-lg hover:bg-gray-50">
                      <div className="flex items-center gap-1.5 min-w-0 flex-1">
                        {line.type === 'saldo' ? (
                          ...
                          <Gift className="w-3 h-3 text-primary-600 flex-shrink-0" />
                        )}
                ...
                <div className="pt-4 border-t">
                  ...
                  {totals.becaUnits > 0 && (
                    <div className="mt-3 p-3 bg-primary-50 border border-gray-200 rounded-xl">
                      <div className="flex items-start gap-2">
