...
const StudyContentDetailPage = () => {
  ...
  const handleSaveVideo = async () => {
    if (!videoForm.title.trim() || !selectedSessionId || !selectedTopicId) return;
    
    // Validar según el modo
    if (videoMode === 'url' && !videoForm.video_url.trim()) return;
    if (videoMode === 'upload' && !selectedVideoFile) return;
  ...
  const openDownloadableModal = (sessionId: number, topic: StudyTopic) => {
    ...
    if (topic.downloadable_exercise) {
      ...
      setDownloadableForm({ title: '', file_url: '', file_name: '' });
    }
    setDownloadableModalOpen(true);
  };

  const handleSaveDownloadable = async () => {
    if (!downloadableForm.title.trim() || !downloadableForm.file_url.trim() || !selectedSessionId || !selectedTopicId) return;
    setSaving(true);
    try {
      await upsertDownloadable(materialId, selectedSessionId, selectedTopicId, downloadableForm);
      await loadMaterial();
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              type="url"
              value={downloadableForm.file_url}
              onChange={(e) => setDownloadableForm({ ...downloadableForm, file_url: e.target.value })}
              className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            ...
            <input
              type="text"
              value={downloadableForm.file_name}
              onChange={(e) => setDownloadableForm({ ...downloadableForm, file_name: e.target.value })}
              className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
              placeholder="ejercicio.pdf"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            <button onClick={() => setDownloadableModalOpen(false)} className="px-4 py-2 border rounded-lg hover:bg-gray-50">
              Cancelar
            </button>
            <button
              onClick={handleSaveDownloadable}
              disabled={saving || !downloadableForm.title.trim() || !downloadableForm.file_url.trim() || !downloadableForm.file_name.trim()}
              className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2"
            >
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
              Guardar
