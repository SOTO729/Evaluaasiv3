...
const Layout = ({ children }: LayoutProps) => {
  ...
  // Polling de mensajes no leídos de chat
  useEffect(() => {
    if (!hasChatAccess) return
    let cancelled = false
    const fetchUnread = async () => {
      try {
        const count = await supportChatService.getUnreadCount()
              ...
              <div className="hidden lg:flex items-center flex-1 min-w-0 fluid-ml-6 relative">
                ...
                {user?.role === 'coordinator' && (
                  <>
                    ...
                    <Link to="/partners/reports" className={`whitespace-nowrap flex-shrink-0 fluid-px-3 fluid-py-1.5 fluid-rounded-lg fluid-text-sm transition-all ${location.pathname === '/partners/reports' ? 'text-primary-600 font-semibold bg-primary-50' : 'text-gray-700 hover:text-primary-600 hover:bg-gray-50'}`}>Reportes</Link>
                  </>
                )}
                ...
                {user?.role !== 'editor' && user?.role !== 'editor_invitado' && user?.role !== 'coordinator' && user?.role !== 'responsable' && user?.role !== 'responsable_partner' && user?.role !== 'gerente' && user?.role !== 'financiero' && user?.role !== 'soporte' && (
                  <Link 
                    to="/certificates" 
                    className={`whitespace-nowrap flex-shrink-0 fluid-px-3 fluid-py-1.5 fluid-rounded-lg fluid-text-sm transition-all ${
                ...
                {user?.role !== 'coordinator' && user?.role !== 'responsable' && user?.role !== 'responsable_partner' && user?.role !== 'gerente' && user?.role !== 'financiero' && user?.role !== 'soporte' && (
                  <Link 
                    to="/exams" 
                    className={`whitespace-nowrap flex-shrink-0 fluid-px-3 fluid-py-1.5 fluid-rounded-lg fluid-text-sm transition-all ${
                ...
                {(user?.role === 'admin' || user?.role === 'developer') && (
                  <Link 
                    to="/tramites-conocer" 
                    className={`whitespace-nowrap flex-shrink-0 fluid-px-3 fluid-py-1.5 fluid-rounded-lg fluid-text-sm transition-all ${
                      location.pathname.startsWith('/tramites-conocer') 
                        ? 'text-primary-600 font-semibold bg-primary-50' 
                        : 'text-gray-700 hover:text-primary-600 hover:bg-gray-50'
                    }`}
                  >
                ...
                {(user?.role === 'admin' || user?.role === 'developer') && (
                  <Link 
                    to="/partners/reports" 
                    className={`whitespace-nowrap flex-shrink-0 fluid-px-3 fluid-py-1.5 fluid-rounded-lg fluid-text-sm transition-all ${
                      location.pathname === '/partners/reports' 
                        ? 'text-primary-600 font-semibold bg-primary-50' 
                        : 'text-gray-700 hover:text-primary-600 hover:bg-gray-50'
                    }`}
                  >
                ...
                {/* ── Bloque responsable: orden personalizado ── */}
                {user?.role === 'responsable' && (
                  <>
                    ...
                    <Link to="/mi-plantel/saldo" className={`whitespace-nowrap flex-shrink-0 fluid-px-3 fluid-py-1.5 fluid-rounded-lg fluid-text-sm transition-all ${location.pathname === '/mi-plantel/saldo' ? 'text-primary-600 font-semibold bg-primary-50' : 'text-gray-700 hover:text-primary-600 hover:bg-gray-50'}`}>Mi Saldo</Link>
                ...
                <div className="absolute right-0 fluid-mt-2 w-[clamp(18rem,16rem+10vw,32rem)] bg-white fluid-rounded-2xl shadow-xl border border-gray-200 fluid-py-3 z-50">
                  ...
                  <div className="fluid-py-2">
                    {(user?.role === 'candidato' || user?.role === 'coordinator') && (
                      ...
                      >
                        <div className="relative fluid-mr-4">
                        ...
                        <span className="flex-1">{user?.role === 'coordinator' ? 'Chat derivado' : 'Chat con Soporte'}</span>
                        {chatUnreadCount > 0 && (
                          <span className="ml-2 min-w-[1.25rem] h-5 flex items-center justify-center bg-red-500 text-white text-xs font-bold rounded-full px-1.5">
          ...
          >
            <nav className="fluid-px-4 fluid-py-3 space-y-1">
              ...
              {(user?.role === 'candidato' || user?.role === 'coordinator') && (
                ...
                >
                  <div className="flex items-center">
                    <div className="relative fluid-mr-3">
                      <svg className="fluid-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {user?.role === 'coordinator' && (
                <>
                  ...
                  <Link to="/partners/reports" className={`block fluid-px-3 fluid-py-3 fluid-rounded-lg transition-all fluid-text-sm ${location.pathname === '/partners/reports' ? 'bg-primary-50 text-primary-600 font-medium' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setIsMobileMenuOpen(false)}>
                    <div className="flex items-center">
              ...
              {/* ── Bloque responsable mobile: orden personalizado ── */}
              {user?.role === 'responsable' && (
                <>
                  <Link to="/mi-plantel" className={`block fluid-px-3 fluid-py-3 fluid-rounded-lg transition-all fluid-text-sm ${location.pathname === '/mi-plantel' ? 'bg-primary-50 text-primary-600 font-medium' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setIsMobileMenuOpen(false)}>
                    <div className="flex items-center">
                      <svg className="fluid-icon fluid-mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
                      Mi Plantel
                    </div>
                  ...
                  <Link to="/study-contents" className={`block fluid-px-3 fluid-py-3 fluid-rounded-lg transition-all fluid-text-sm ${location.pathname.startsWith('/study-contents') ? 'bg-primary-50 text-primary-600 font-medium' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setIsMobileMenuOpen(false)}>
                    <div className="flex items-center">
                      <svg className="fluid-icon fluid-mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
                      Materiales
                    </div>
                  </Link>
                  <Link to="/certificates" className={`block fluid-px-3 fluid-py-3 fluid-rounded-lg transition-all fluid-text-sm ${location.pathname.startsWith('/certificates') ? 'bg-primary-50 text-primary-600 font-medium' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setIsMobileMenuOpen(false)}>
                    <div className="flex items-center">
                      <svg className="fluid-icon fluid-mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></svg>
                      Certificados
                    </div>
                  </Link>
                  <Link to="/mi-plantel/saldo" className={`block fluid-px-3 fluid-py-3 fluid-rounded-lg transition-all fluid-text-sm ${location.pathname === '/mi-plantel/saldo' ? 'bg-primary-50 text-primary-600 font-medium' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setIsMobileMenuOpen(false)}>
                    <div className="flex items-center">
