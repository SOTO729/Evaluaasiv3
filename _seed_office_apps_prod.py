"""Seed/upsert OfficeAppVersion in PROD for the 3 Office 2019 exams.

Idempotent: actualiza si existe, inserta si no.
"""
import pymssql
...
DATABASE = 'evaluaasi'  # PROD

BASE_URL = 'https://evaluaasimotorv2storage.blob.core.windows.net/vb6-downloads/prod'
...
def main():
    ...
    for app_name, app_type, minv, latest, url, active in ENTRIES:
        ...
        if existing:
            ...
            print(f'UPDATED  {app_name}  (was id={existing["id"]})  min={minv} latest={latest}')
        else:
            cur.execute(
            ...
            print(f'INSERTED {app_name}  min={minv} latest={latest}')
    conn.commit()
